...
export const getCurrentTime = () => {
    ...
    let currentTime = {year, month, day, hour, minute, second, weekday: weekday[time.getDay()]};
    return currentTime;
};
...
export const checkDays = (mourn) => {
    const currentDate = new Date();
    const month = currentDate.getMonth() + 1;
    const day = currentDate.getDate();
    const key = `${month}.${day}`;
    let mournSwitch = mourn.mournSwitch;
    let mournText = mourn.mournText;
    if (mournSwitch || anniversaries.hasOwnProperty(key)) {
        if (!mournSwitch) mournText = `今天是${anniversaries[key]}`;
        console.log(mournText);
...
export const siteDateStatistics = (startDate) => {
    ...
    if (differenceInYears >= 1) {
        return `本站已经运行了 ${Math.floor(differenceInYears)} 年 ${Math.floor(differenceInMonths % 12)} 月 ${Math.round(differenceInDays % 30)} 天`;
    } else if (differenceInMonths >= 1) {
        return `本站已经运行了 ${Math.floor(differenceInMonths)} 月 ${Math.round(differenceInDays % 30)} 天`;
    } else {
